...
/**
 * This implementation of {@link RecyclerView.ItemAnimator} provides basic animations on remove, add, and move events
 * that happen to the items in a RecyclerView. RecyclerView uses a DefaultItemAnimator by default.
 * 
...
public class DefaultItemAnimator extends RecyclerView.ItemAnimator {
    ...
    private final ArrayList<ViewHolder> mAdditions = new ArrayList<ViewHolder>();
    ...
    public void runPendingAnimations() {
        ...
        if (movesPending) {
            ...
            if (removalsPending) {
                final View view = mMoves.get(0).holder.itemView;
        ...
        if (additionsPending) {
            ...
            if (removalsPending || movesPending) {
                final View view = mAdditions.get(0).itemView;
    ...
    private void animateRemoveImpl(final ViewHolder holder) {
        ...
        ViewCompat.animate(view).setDuration(getRemoveDuration()).alpha(0).setListener(new VpaListenerAdapter() {
            @Override
    ...
    private void animateAddImpl(final ViewHolder holder) {
        final View view = holder.itemView;
        ViewCompat.animate(view).cancel();
        ViewCompat.animate(view).alpha(1).setDuration(getAddDuration()).setListener(new VpaListenerAdapter() {
            @Override
    ...
    /**
     * Check the state of currently pending and running animations. If there are none pending/running, call
     * {@link #dispatchAnimationsFinished()} to notify any listeners.
     */
    ...
    public void endAnimations() {
        ...
        for (int i = count - 1; i >= 0; i--) {
            final ViewHolder item = mMoveAnimations.get(i);
            final View view = item.itemView;
            ViewCompat.animate(view).cancel();
            ViewCompat.setTranslationY(view, 0);
            ViewCompat.setTranslationX(view, 0);
        ...
        for (int i = count - 1; i >= 0; i--) {
            final ViewHolder item = mRemoveAnimations.get(i);
            final View view = item.itemView;
            ViewCompat.animate(view).cancel();
            ViewCompat.setAlpha(view, 1);
            dispatchRemoveFinished(item);
        ...
        for (int i = count - 1; i >= 0; i--) {
            final ViewHolder item = mAddAnimations.get(i);
            final View view = item.itemView;
            ViewCompat.animate(view).cancel();
            ViewCompat.setAlpha(view, 1);
            dispatchAddFinished(item);
